...
def alimentar():
    ...
    while True:
        ...
        if total == 0:
            time.sleep(2)
            print("\n" "Seu pet está faminto, alimente-o o mais rápido possível!!!")
            break

def banho():
    total = 100
    while True:
        acao = input("Aumentando sujeira... Tomar banho? [0 - NÃO / 1 - SIM] (ou 's' para sair): ")
        ...
        if acao == '1':
            ...
            if opcao == 1:
                print("Sabonete diminui 10 de sujeira")
                try:
                    sabonete = int(input("Deseja usar sabonete? [0 - NÃO / 1 - SIM]: "))
                except ValueError:
                    print("Entrada inválida. Digite 0 para NÃO ou 1 para SIM.")
                    continue
                ...
                print("Shampoo diminui 25 de sujeira")
                try:
                    shampoo = int(input("Deseja usar Shampoo? [0 - NÃO / 1 - SIM]: "))
                except ValueError:
                    print("Entrada inválida. Digite 0 para NÃO ou 1 para SIM.")
                    continue
                ...
                print("Condicionador diminui 50 de sujeira")
                try:
                    condicionador = int(input("Deseja usar condicionador? [0 - NÃO / 1 - SIM]: "))
                except ValueError:
                    print("Entrada inválida. Digite 0 para NÃO ou 1 para SIM.")
                    continue
        ...
        if total == 0:
            time.sleep(2)
            print("\n" "Seu pet está muito sujo, higienize-o o mais rápido possível!!!")
            break
...
def cadastrar_usuario():
    with open('usuarios.txt', 'a') as arquivo:
        arquivo.write('usuario,senha,nome_pet\n')

    usuario = input("Digite o nome do usuário: ")
    senha = input("Digite a senha do usuário: ")
    nome_pet = input("Digite o nome do seu pet virtual: ")
    ...
    with open("usuarios.txt", "a") as arquivo:
        arquivo.write(f"{usuario}:{senha}:{nome_pet}\n")
    ...
    with open("usuarios.txt", "r") as arquivo:
        ...
        for linha in linhas:
            ...
            if usuario_login == usuario_usuario and senha == usuario_senha:
                print("Login bem-sucedido!")
                if nome_pet:
                    print("Seu pet virtual:", nome_pet)
                return
...
def jogo_forca():
    palavra_secreta = "manguebeat"
    letras_corretas = []
    tentativas = 6

    while True:
        print(f'Bem-vindo ao jogo da forca do Xodó de bolso!')
        ...
        if palpite in palavra_secreta:
            ...
            tentativas -= 1
            print(f"Letra incorreta! Você tem mais {tentativas} tentativas.")

def jogo_perguntas():
    perguntas = [
        {
            "pergunta": "Qual cidade brasileira é considerada o berço do movimento Manguebeat?",
            "opcoes": ["Recife", "Salvador", "Rio de Janeiro", "São Paulo"],
            "resposta": "Recife"
        },
        {
            "pergunta": "Quem é conhecido como o principal idealizador e líder do Manguebeat?",
            "opcoes": ["Chico Science", "Gilberto Gil", "Caetano Veloso", "Lenine"],
            "resposta": "Chico Science"
        },
        {
            "pergunta": "Qual banda ficou famosa por misturar ritmos como maracatu e punk rock no Manguebeat?",
            "opcoes": ["Nação Zumbi", "Mundo Livre S/A", "Eddie", "Cordel do Fogo Encantado"],
            "resposta": "Nação Zumbi"
        },
        {
            "pergunta": "Qual o nome do primeiro álbum lançado por Chico Science & Nação Zumbi?",
            "opcoes": ["Da Lama ao Caos", "Afrociberdelia", "Sobrevivendo no Inferno", "A Revolta dos Ratos"],
            "resposta": "Da Lama ao Caos"
        },
        {
            "pergunta": "Qual instrumento musical era característico do Manguebeat?",
            "opcoes": ["Alfaia", "Guitarra", "Violino", "Flauta"],
            "resposta": "Alfaia"
        }
    ]
    ...
    for pergunta in perguntas:
        resposta = exibir_pergunta(pergunta["pergunta"], pergunta["opcoes"])
    ...
    print(f"Fim do jogo! Você marcou {pontos} pontos.")

def menu_minijogos():
    print("Bem-vindo aos minijogos!")
    while True:
        opcao = input(f'''
    MINIJOGOS XODÓ DE BOLSO
